...
export class ParsianServer {
	private wss: ws.WebSocketServer | null = null;
	private port: number = 0;
	private lockFilePath: string = '.parsian-lock';
	private browserClients: Set<ws.WebSocket> = new Set();
	private vscodeClients: Set<ws.WebSocket> = new Set();
	...
	public async start(): Promise<void> {
		try {
			...
			this.wss.on('connection', (socket: ws.WebSocket, request) => {
	...
	private handleMessage(socket: ws.WebSocket, message: ws.Data): void {
		try {
	...
	private handleCodeSuggestion(socket: ws.WebSocket, request: ParsianRequest): void {
		...
		const responseData: ParsianResponse = {
...
function exampleFunction() {
  ...
  return true;
	...
	private handleExplanationRequest(socket: ws.WebSocket, request: ParsianRequest): void {
	...
	private handleCommandRequest(socket: ws.WebSocket, request: ParsianRequest): void {
	...
	private sendError(socket: ws.WebSocket, requestId: string, message: string): void {
		const errorResponse = {
	...
	private sendMessageWithErrorHandling(socket: ws.WebSocket, message: any): boolean {
		try {
	...
	private handleAIResponse(socket: ws.WebSocket, data: any): void {
	...
	private handleRateLimitDetected(socket: ws.WebSocket, data: any): void {
	...
	private handleCaptchaDetected(socket: ws.WebSocket, data: any): void {
	...
	public broadcastMessage(message: any): void {
		...
		this.vscodeClients.forEach((client: ws.WebSocket) => {
	...
	public sendMessageToClient(client: ws.WebSocket, message: any): void {
		if (client.readyState === ws.WebSocket.OPEN) {
	...
	public sendRequestToBrowserExtension(request: any): boolean {
		if (this.browserClients.size === 0) {
