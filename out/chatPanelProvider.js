...
class ChatPanelProvider {
    constructor(panel, extensionUri, server) {
        ...
        this._server = server;
        this._contextManager = new contextManager_1.ContextManager();
    ...
    static createOrShow(extensionUri, server) {
        ...
        const panel = vscode.window.createWebviewPanel('parsianChat', 'The Parsian AI Developer Agent', column || vscode.ViewColumn.One, {
            enableScripts: true,
            retainContextWhenHidden: true
        });
        ChatPanelProvider.currentPanel = new ChatPanelProvider(panel, extensionUri, server);
    }
    static revive(panel, extensionUri, server) {
        ChatPanelProvider.currentPanel = new ChatPanelProvider(panel, extensionUri, server);
    }
    dispose() {
    ...
    async handleProjectScanRequest() {
...
This project contains ${this.countFiles(projectStructure)} files and ${this.countDirectories(projectStructure)} directories.`;
    ...
    scanProjectStructure(rootPath, maxDepth = 3) {
        try {
            const result = [];
    ...
    scanDirectory(dirPath, result, currentDepth, maxDepth, rootPath) {
    ...
    handleApplyCode(code, language) {
        ...
        diffApplier_1.DiffApplier.applyCodeChange(code, language);
    }
    _update() {
