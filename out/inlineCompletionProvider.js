...
class InlineCompletionProvider {
    provideInlineCompletionItems(document, position, _context, _token) {
        ...
        if (lineText.trimEnd().endsWith('//')) {
            ...
            const item = new vscode.InlineCompletionItem(completionText, new vscode.Range(position, position.translate(0, completionText.length)));
            return [item];
        }
        ...
        if (lineText.includes('ai:')) {
            ...
            const item = new vscode.InlineCompletionItem(completionText, new vscode.Range(position.with(undefined, startIndex), position.with(undefined, endIndex)));
            return [item];
        }
